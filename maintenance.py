...
def test_youtube_links(html_file: str) -> bool:
  ...
  logger.info("Link validation finished. Valid: %d, Broken: %d",
...
def run_benchmark(html_file: str) -> None:
  ...
  albums = extract_album_entries_from_html(html_file)
  video_ids = [
      info["video_id"] for info in albums.values() if info.get("video_id")
  ]
  ...
  logger.info("Benchmarking image loading times for %d posters...",
              len(video_ids))
  ...
  hq_urls = [
      f"https://i.ytimg.com/vi/{vid}/hqdefault.jpg" for vid in video_ids
  ]
  ...
  print(f"  Avg Image Size:       {sum(maxres_sizes) / len(maxres_sizes) / 1024:.1f} KB")
  print(f"  Avg Fetch Time:       {sum(maxres_times) / len(maxres_times):.3f} seconds")
...
def export_tsv_glossary(html_file: str, tsv_file: str) -> None:
  ...
  logger.info("Exporting %d albums to TSV glossary '%s'...",
              len(albums), tsv_file)
...
def main() -> None:
  ...
  match_parser = subparsers.add_parser(
      "match", help="Match albums on YouTube Music"
  )
  ...
  if args.command == "match":
    try:
      ...
      logger.info("Failed to load header file. Initializing in unauthenticated mode.")
      yt = YTMusic()
    ...
    if args.search:
      ...
      if match:
        logger.info("Match found: PlaylistID=%s, VideoID=%s",
                    match.get("playlistId"), match.get("firstVideoId"))
      else:
